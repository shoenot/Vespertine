...
use vespertine_abi::app::hesper::{
    AppIoMode, HESPER_STATUS_INVALID_REQUEST, HESPER_STATUS_NOT_FOUND, HESPER_STATUS_OK,
};
use vespertine_std::{
    Error, Write,
    hesper::{
        HesperRequest, decode_io_mode_string, send_app_metadata_response,
...
pub fn handle_request(
    socket: &Socket,
    request: HesperRequest,
    log: &SystemLog,
) -> Result<(), Error> {
    match request {
        HesperRequest::AppMetadata {
            request_id,
            request,
        } => match get_metadata(&request.app_name) {
            Ok(metadata) => {
            ...
            Err(error) => {
                log.write_string(format!(
                    "metadata lookup failed for {}: {:?}",
                    request.app_name, error
                ))?;
                send_launcher_not_found(socket, request_id);
                return Err(error);
            }
        },
        HesperRequest::Execute {
            request_id,
            request,
        } => {
            log.write_string(format!(
                "execute requested: {} with {} arguments",
                request.app_name,
...
fn send_launcher_not_found(sock: &Socket, id: u32) {
    let _ = send_app_metadata_response(
        &sock,
        id,
