...
impl<'a> Opt<'a> {
    pub const fn flag(id: &'a str, short: Option<char>, long: Option<&'a str>) -> Self { Self { id, short, long, value: ValueMode::None } }
...
impl<'a> Command<'a> {
    pub const fn new(name: &'a str) -> Self { Self { name, options: &[], allow_positionals: true } }
    ...
    fn find_short(&self, ch: char) -> Option<&Opt<'a>> { self.options.iter().find(|opt| opt.short == Some(ch)) }

    fn find_long(&self, name: &str) -> Option<&Opt<'a>> { self.options.iter().find(|opt| opt.long == Some(name)) }
...
impl<'a> Matches<'a> {
    pub fn flag(&self, id: &str) -> bool { self.options.iter().any(|opt| opt.id == id) }

    pub fn value(&self, id: &str) -> Option<&'a str> { self.options.iter().find(|opt| opt.id == id).and_then(|opt| opt.value) }
    ...
    pub fn positionals(&self) -> &[&'a str] { &self.positionals }

    pub fn positional(&self, index: usize) -> Option<&'a str> { self.positionals.get(index).copied() }

    pub fn require_positional(&self, index: usize, name: &str) -> Result<&'a str, CliError> {
        self.positional(index).ok_or_else(|| CliError::MissingArgument(name.to_string()))
    }

    pub fn positional_count(&self) -> usize { self.positionals.len() }
