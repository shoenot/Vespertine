use vespertine_abi::tag::CAP_CLOCK;
use vespertine_abi::{
    AccessRights,
    HandleID,
};
use vrt::once::OnceCell;
use vrt::syscall::{
    sys_close,
    sys_get_time,
    sys_sleep,
...
impl Clock {
    ...
    pub fn sleep_ms(&self, ms: usize) -> Result<(), Error> { sys_sleep(ms, self.handle).map_err(|e| Error::from(e)) }

    pub fn now(&self) -> (usize, usize) { sys_get_time(self.handle) }
}
...
impl Time {
    pub fn now() -> (usize, usize) {
        let clock =
            READ_CLOCK.get_or_init(|| Clock::request(AccessRights::READ).expect("Program does not have the appropriate Clock capability"));
        clock.now()
    }

    pub fn sleep_ms(ms: usize) -> Result<(), Error> {
        let clock = WRITE_CLOCK
            .get_or_init(|| Clock::request(AccessRights::WRITE).expect("Program does not have the appropriate Clock capability"));
        clock.sleep_ms(ms)
    }
}
