...
impl<T> OnceCell<T> {
    pub const fn new() -> Self { Self { state: AtomicU32::new(UNINITIALIZED), value: UnsafeCell::new(MaybeUninit::uninit()) } }

    pub fn get(&self) -> Option<&T> {
        if self.state.load(Ordering::Acquire) == INITIALIZED { Some(unsafe { (*self.value.get()).assume_init_ref() }) } else { None }
    }
        ...
        F: FnOnce() -> Result<T, E>,
    {
        ...
        loop {
            match self.state.compare_exchange(UNINITIALIZED, INITIALIZING, Ordering::Acquire, Ordering::Acquire) {
                Ok(_) => {
                    let result = initialize.take().expect("initializer called more than once")();
